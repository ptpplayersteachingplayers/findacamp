...
class PTP_Shop_Component {
    ...
    /**
     * Shortcode: [ptp_find_camp gmaps_key="..." endpoint="/wp-json/ptp/v1/winter-products"]
     * Renders the Google Maps + ZIP Finder component.
     */
    public function render_find_camp_shortcode($atts) {
        $atts = shortcode_atts(array(
            'gmaps_key' => get_option('ptp_gmaps_key', ''),
        ...
        <section id="ptp-find-camp" class="alignfull" aria-label="Find a PTP Camp Near You">
          ...
          (function(){
            ...
            async function fetchProducts(){
              ...
              diag.ep.textContent = ep; const data = await j(ep);
            ...
            // Coords resolution hierarchy: venue lat/lng → loc_tag → tag-lookup
            ...
            function demo(){ return [
              ...
              { name:'PTP Select Camp — Scarsdale (Demo)', permalink:'#scarsdale', prices:{price:49900,currency_code:'USD'}, images:[{src:'https://ptpsummercamps.com/wp-content/uploads/2025/09/BG7A7342-scaled.jpg'}], tags:[{slug:'scarsdale',name:'Scarsdale'}], venue:{lat:'',lng:'',name:''}, loc_tag:'scarsdale' }
            ]; }
            btnDemo.addEventListener('click', async ()=>{ currentProducts = demo(); lastUserCoords = loadCoords(); render(currentProducts,lastUserCoords); await loadMaps(); updateMap(currentProducts,lastUserCoords); setStatus('Demo loaded — layout & map verified.'); });
    ...
    /**
     * Register custom REST API routes for the Find a Camp component.
     */
    public function register_rest_routes() {
        register_rest_route('ptp/v1', '/winter-products', array(
            'methods' => 'GET',
            'callback' => array($this, 'rest_winter_products'),
            ...
            'args' => array(
                ...
                // Optional: override category slug (defaults to winter-clinics)
                'category' => array(
                    'description' => 'Product category slug to filter',
                    'type' => 'string',
                    'default' => 'winter-clinics'
                )
            )
        ));
    }
    ...
    /**
     * REST callback: returns simplified product data suitable for the map + ZIP component.
     */
    public function rest_winter_products( $request ) {
        ...
        $per_page = (int) ( $request['per_page'] ?? 200 );
        $per_page = max(1, min(500, $per_page));
        $category = sanitize_title( (string) ( $request['category'] ?? 'winter-clinics' ) );

        // Query Woo products by category slug; include visible, published products only
        $args = array(
            'status'   => 'publish',
            'limit'    => $per_page,
            'orderby'  => 'date',
        ...
        // If the winter category doesn't exist yet, fall back to returning recent products
        ...
        foreach ($products as $product) {
            $items[] = $this->map_product_to_item($product, $currency);
        }
    ...
    /**
     * Transform a WC_Product into the lightweight JSON structure expected by the frontend.
     */
    private function map_product_to_item( $product, $currency = 'USD' ) {
        $id = $product->get_id();

        // Price in cents
        $price_float = function_exists('wc_get_price_to_display') ? wc_get_price_to_display($product) : floatval($product->get_price());
        ...
        if ($img_id) {
            ...
            if ($src && !empty($src[0])) $img_src = $src[0];
        }
        ...
        // Venue coordinates from common custom meta keys (optional)
        $venue_lat = get_post_meta($id, 'venue_lat', true);
        $venue_lng = get_post_meta($id, 'venue_lng', true);
        if ($venue_lat === '') $venue_lat = get_post_meta($id, 'pa_venue_lat', true);
        if ($venue_lng === '') $venue_lng = get_post_meta($id, 'pa_venue_lng', true);
        $venue_name = get_post_meta($id, 'venue_name', true);
        $venue_addr = get_post_meta($id, 'venue_address', true);
        $venue = array();
        if ($venue_lat !== '' && $venue_lng !== '') {
            $venue = array(
                'lat' => is_numeric($venue_lat) ? (float) $venue_lat : $venue_lat,
                'lng' => is_numeric($venue_lng) ? (float) $venue_lng : $venue_lng,
                'name' => $venue_name,
                'address' => $venue_addr,
            );
        }

        // Optionally expose a recognized loc_tag based on known tag slugs
        $loc_tag = '';
        $known = $this->known_location_slugs();
        foreach ($tags as $t) {
            if (in_array(strtolower($t['slug']), $known, true)) { $loc_tag = strtolower($t['slug']); break; }
        }
        ...
        return array(
            ...
            'images' => $img_src ? array(array('src' => $img_src)) : array(),
            'tags' => $tags,
            'venue' => $venue,
            'loc_tag' => $loc_tag,
        );
    ...
    /**
     * The set of location slugs recognized by the front-end for mapping.
     */
    private function known_location_slugs() {
        static $slugs = null;
        if ($slugs !== null) return $slugs;
        $slugs = array(
            'main-line','west-chester','doylestown','media','princeton','short-hills','ridgewood','hockessin','greenville','scarsdale','rye','garden-city'
        );
        return $slugs;
    }
